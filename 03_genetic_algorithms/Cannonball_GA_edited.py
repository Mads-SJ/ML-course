...
def fitness(generation, width):
    results = [hit_coordinate(theta, v, width) for (theta, v) in generation]
    print(results)
    return results

def get_best(generation, width, percentage):
    results = fitness(generation, width)
    # sort by y_hit
    results = sorted(results, key=lambda x: x[1], reverse=True)
    number_of_elites = math.floor(len(generation) * percentage)
    return results[:number_of_elites]
...
def crossover(generation, width):
    choices = get_best(generation, width, 0.2)
    next_generation = choices
    for i in range(len(choices), len(generation)):
        mum = choose2(choices)
        dad = choose2(choices)
...
def display_generations(generations, height, width):
    matplotlib.rcParams.update({'font.size': 16})
    fig = plt.figure()
    plt.subplots_adjust(hspace=2)
    for i, generation in enumerate(generations):
        ax = fig.add_subplot(len(generations), 1, i + 1)
...
def fire():
    ...
    #display_generations(generations, height, width)
